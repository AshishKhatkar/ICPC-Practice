...
vector <string> BinaryCode::decode(string message) {
	...
	for(int i = 0; i < len; ++i) {
		...
		val2 += (res1[i] - '0');
		int res = val1 - val2;
		if((res == 0 || res == 1) && i != len - 1) {
			res1.push_back((char)(res + '0'));
		} else if(i == len - 1){
			if(val1 != val2) {
				flag1 = false;
			}
	...
	if(flag2) {
		for(int i = 0; i < len; ++i) {
			final_res2 = final_res2 + res2[i];
		}
	} else {
		final_res2 = "NONE";
	}
	cout << final_res1 << " " << final_res2 << endl;
	vector<string> v;
	v.push_back(final_res1);
	v.push_back(final_res2);
	return v;
}
